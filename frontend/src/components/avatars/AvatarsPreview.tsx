...
function PlaceholderAvatar({ palette, accent }: { palette: string; accent: string }) {
  return (
      ...
      <Card className="border-brand-100/70 bg-gradient-to-br from-white via-white to-brand-50/60 p-6 sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-primary">Create videos with your branded AI hosts</h1>
            <p className="max-w-2xl text-sm text-slate-600 sm:text-base">
              We&apos;re building a library of camera-ready avatars with configurable voice, pacing, and personality presets.
              Here&apos;s a look at the upcoming experience.
            </p>
          </div>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {avatarCards.map((avatar) => (
          <Card key={avatar.name} className="space-y-4 p-4 sm:p-5">
            <PlaceholderAvatar palette={avatar.palette} accent={avatar.accent} />
            <div>
