...
export function Videos() {
  ...
  const autoCaptionCacheRef = useRef<Record<string, string>>({})
  ...
  const handleAutoGenerateDescription = async (video: VideoRecord) => {
    if (!video) return

    const cachedCaption = autoCaptionCacheRef.current[video.id]
    if (cachedCaption) {
      setPostDescription(cachedCaption)
      return
    }
    ...
    try {
      ...
      if (mountedRef.current && postModalVideoIdRef.current === video.id) {
        autoCaptionCacheRef.current[video.id] = description
        setPostDescription(description || video.topic)
      }
    } catch (error) {
      console.error('Failed to auto-generate description:', error)
      if (mountedRef.current && postModalVideoIdRef.current === video.id) {
        setPostDescription(video.topic)
      }
    } finally {
      if (mountedRef.current) {
  ...
  const handleOpenPostModal = (video: VideoRecord) => {
    postModalVideoIdRef.current = video.id
    setSelectedVideoForPost(video)
    const cachedCaption = autoCaptionCacheRef.current[video.id]
    setPostDescription(cachedCaption || video.topic)
    setIsPostModalOpen(true)
    void handleAutoGenerateDescription(video)
  }
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      autoCaptionCacheRef.current[selectedVideoForPost.id] = description
  ...
  const handleClosePostModal = () => {
    ...
    postModalVideoIdRef.current = null
  }
