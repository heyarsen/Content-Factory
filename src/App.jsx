...
const SocialAccountManager = () => {
  ...
  const connectSocialAccounts = async () => {
    ...
    try {
      ...
      // Create user with upload-post.com via proxy
      const createResponse = await fetch('/api/proxy/uploadpost/users', {
        method: 'POST',
        headers: {
      ...
      if (profileCreated) {
        ...
        // Generate JWT for connecting accounts via proxy
        setDebugInfo('Generating JWT...');
        
        const jwtResponse = await fetch('/api/proxy/uploadpost/users/generate-jwt', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
...
// Root App with Provider - THIS IS THE DEFAULT EXPORT
